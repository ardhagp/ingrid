    ...
    Public Class Upload
        ...
        Public Shared Function IsAllowedSize(filepath As String, maxsize As Double, Optional shownativedialog As Boolean = False) As Boolean
            ...
            If varOriginalPhotoLength > varMaximumPhotoLength Then
                Dim varErrorBox As New CMCv.FRMdialogbox("Your file size is : " & String.Format("{0:0.##} {1}", varPhotoLength, size(varOrder)) & ", above the size that we can tolerate." & Environment.NewLine & "Please pick your file size under " & String.Format("{0:0.##} {1}", varMaximumPhotoLengthDisplay, size(varMaximumOrder)) & " and then try again.", "Limit Exceeded", FRMdialogbox.MessageIcon.Error, FRMdialogbox.MessageTypes.OkOnly)
