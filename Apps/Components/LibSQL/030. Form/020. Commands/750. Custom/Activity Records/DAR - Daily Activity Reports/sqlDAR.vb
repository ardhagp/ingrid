...
Namespace CMDdar
    Public Class View
        ...
        ' Utility helpers extracted to reduce complexity
        Private Shared Function SafeTrim(s As String) As String
            If s Is Nothing Then Return String.Empty
            Return s.Trim()
        End Function

        Private Shared Function EscapeSql(s As String) As String
            If s Is Nothing Then Return String.Empty
            Return s.Replace("'", "''")
        End Function

        Private Shared Function SplitTerms(s As String) As String()
            If String.IsNullOrWhiteSpace(s) Then
                Return Array.Empty(Of String)()
            End If
            Return s.Split(New String() {"||"}, StringSplitOptions.RemoveEmptyEntries).Select(Function(x) x.Trim()).Where(Function(x) x <> String.Empty).ToArray()
        End Function

        Private Shared Function BuildLikeCondition(field As String, search As String) As String
            search = SafeTrim(search)
            If search = String.Empty Then Return String.Empty

            Dim tokens = SplitTerms(search)
            If tokens.Length = 0 Then Return String.Empty

            If tokens.Length = 1 Then
                Return $"({field} LIKE '%{EscapeSql(tokens(0))}%')"
            End If

            Dim parts As New List(Of String)
            For Each t In tokens
                parts.Add($"{field} LIKE '%{EscapeSql(t)}%'")
            Next
            Return "(" & String.Join(" AND ", parts) & ")"
        End Function

        Private Shared Function YearMonthClauseForEngine(dbengine As LibApp.Ingrid.Global.DatabaseEngine, chkdatefilterChecked As Boolean, dtpdatefilter As CMCv.UI.Control.dtp) As String
            If Not chkdatefilterChecked Then
                If dbengine = LibApp.Ingrid.Global.DatabaseEngine.MSSQL Then
                    Return "(year(ea.employeeactivity_datetime) = year(getdate())) And (month(ea.employeeactivity_datetime) = month(getdate()))"
                Else
                    Return "(year(ea.employeeactivity_datetime) = year(now())) And (month(ea.employeeactivity_datetime) = month(now()))"
                End If
            Else
                Return $"(year(ea.employeeactivity_datetime) = {dtpdatefilter.Value.Year} And month(ea.employeeactivity_datetime) = {dtpdatefilter.Value.Month})"
            End If
        End Function
        ...
        Public Shared Sub DisplayMainGrid(databasename As String, dbengine As LibApp.Ingrid.Global.DatabaseEngine, find As CMCv.UI.Control.txt, dategrid As CMCv.UI.Control.dgn, datestatusbar As CMCv.UI.Control.stt, contentstatusbar As CMCv.UI.Control.stt, chkdatefilter As CMCv.UI.Control.chk, dtpdatefilter As CMCv.UI.Control.dtp, chkbyfilter As CMCv.UI.Control.chk, cbobyfilter As CMCv.UI.Control.cbo, Optional forcerefresh As Boolean = False)
            Try
                ' Cache selected employee id once
                varEmployeeID = If(cbobyfilter Is Nothing OrElse cbobyfilter.Items.Count = 0, "-1", Convert.ToString(cbobyfilter.SelectedValue()))

                Dim whereParts As New List(Of String)

                Dim search = SafeTrim(If(find?.XOSQLText, String.Empty))

                If String.IsNullOrEmpty(search) AndAlso forcerefresh Then
                    ' date-range default
                    whereParts.Add(YearMonthClauseForEngine(dbengine, chkdatefilter.Checked, dtpdatefilter))
                    If chkbyfilter.Checked Then whereParts.Add($"(ea.employeeactivity_employee = '{EscapeSql(varEmployeeID)}')")
                Else
                    ' search clause
                    Dim searchCond = BuildLikeCondition("ea.employeeactivity_description", search)
                    If searchCond <> String.Empty Then whereParts.Add(searchCond)

                    ' optional date filter
                    If chkdatefilter.Checked Then
                        whereParts.Add($"(year(ea.employeeactivity_datetime) = {dtpdatefilter.Value.Year} and month(ea.employeeactivity_datetime) = {dtpdatefilter.Value.Month})")
                    End If
                ...
                ElseIf dbengine = LibApp.Ingrid.Global.DatabaseEngine.MYSQL Then
                    varDatabaseRequestMysql(0).Query = $"select ea.employeeactivity_datetime, (convert(varchar,ea.employeeactivity_datetime,106) + '' + char(13) + char(10) + '' + left(datename(dw,ea.employeeactivity_datetime),3)) as `employeeactivity_longdate` from doc_employeeactivity ea {varWhere} group by ea.employeeactivity_datetime order by ea.employeeactivity_datetime desc"
        ...
        Public Shared Sub DisplaySecondGrid(databasename As String, dbengine As LibApp.Ingrid.Global.DatabaseEngine, dategrid As String, contentgrid As CMCv.UI.Control.dgn, contentstatusbar As CMCv.UI.Control.stt, find As CMCv.UI.Control.txt, Optional showattachment As Boolean = False, Optional photogrid As CMCv.UI.Control.dgn = Nothing, Optional filegrid As CMCv.UI.Control.dgn = Nothing)
            Try
                Dim varContentDateAsString As String = dategrid
                Dim whereParts As New List(Of String)
                whereParts.Add($"(ea.employeeactivity_datetime = '{EscapeSql(varContentDateAsString)}')")
                ...
                If varIsEmpFilter Then
                    whereParts.Add($"(ea.employeeactivity_employee = '{EscapeSql(varEmployeeID)}')")
                End If

                Dim varWhere As String = If(whereParts.Count > 0, "where " & String.Join(" and ", whereParts), String.Empty)

                ' Build time/description fragments once
                ...
                If dbengine = LibApp.Ingrid.Global.DatabaseEngine.MSSQL Then
                    ReDim varDatabaseRequestMssql2008(3)
                    varDatabaseRequestMssql2008(2).Query = String.Format("select aa.areaaffected_name, {1}, {2}, case when (ea.employeeactivity_lastupdate is not null) and (ea.employeeactivity_employee <> ea.employeeactivity_lastupdate) then (convert(varchar(max), e.employee_nickname) + ' / ' + convert(varchar(max), (select em.employee_nickname from dbo.man_employee em where em.employee_id = ea.employeeactivity_lastupdate))) else e.employee_nickname end as [employee_nickname], e.employee_id, ea.employeeactivity_id from dbo.doc_employeeactivity ea inner join dbo.doc_areaaffected aa on ea.employeeactivity_areaaffected = aa.areaaffected_id inner join dbo.man_employee e on ea.employeeactivity_employee = e.employee_id {0} order by aa.areaaffected_order, ea.employeeactivity_time", varWhere, varTimeFormat2, varDescription)
                    ...
                    If Not showattachment Then Return

                    varContentID = If(varDatabaseRequestMssql2008(2).DataGrid.RowCount = 0, "-1", varDatabaseRequestMssql2008(2).DataGrid.CurrentRow.Cells("employeeactivity_id").Value.ToString)
                    DisplayPhotoGrid(databasename, dbengine, varContentID, photogrid)
                    DisplayFileGrid(databasename, dbengine, varContentID, filegrid)
                ElseIf dbengine = LibApp.Ingrid.Global.DatabaseEngine.MYSQL Then
                    ReDim varDatabaseRequestMysql(3)
                    varDatabaseRequestMysql(2).Query = $"select aa.areaaffected_name, {varTimeFormat2}, {varDescription}, case when (ea.employeeactivity_lastupdate is not null) and (ea.employeeactivity_employee <> ea.employeeactivity_lastupdate) then (convert(varchar(max), e.employee_nickname) + ' / ' + convert(varchar(max), (select em.employee_nickname from man_employee em where em.employee_id = ea.employeeactivity_lastupdate))) else e.employee_nickname end as `employee_nickname`, e.employee_id, ea.employeeactivity_id from doc_employeeactivity ea inner join doc_areaaffected aa on ea.employeeactivity_areaaffected = aa.areaaffected_id inner join man_employee e on ea.employeeactivity_employee = e.employee_id {varWhere} order by aa.areaaffected_order, ea.employeeactivity_time"
                    ...
                    varContentID = If(varDatabaseRequestMysql(2).DataGrid.RowCount = 0, "-1", varDatabaseRequestMysql(2).DataGrid.CurrentRow.Cells("employeeactivity_id").Value.ToString)
                    DisplayPhotoGrid(databasename, dbengine, varContentID, photogrid)
                    DisplayFileGrid(databasename, dbengine, varContentID, filegrid)
                End If
            Catch ex As Exception
        ...
        ' The rest of the class (DisplayPhotoGrid, DisplayFileGrid, GetPdfFile, DeleteData, IsLike, LikePhoto, Editor nested class etc.)
        ' remain unchanged to preserve behavior. They are left as-is below to keep this patch focused on reducing complexity
        ' of the large DisplayMainGrid / DisplaySecondGrid methods.
        ...
        Public Shared Sub DisplayPhotoGrid(databasename As String, dbengine As LibApp.Ingrid.Global.DatabaseEngine, contentid As String, filegrid As CMCv.UI.Control.dgn, Optional recordyear As String = "")
            ...
            If dbengine = LibApp.Ingrid.Global.DatabaseEngine.MSSQL Then
                varDatabaseRequestMssql2008(4).Query = String.Format("select fi.file_id, fi.file_filename, fi.file_content, (convert(varchar(25),fi.file_content_size) + ' KB') as [file_content_size], (convert(varchar(3),fi.file_score) + ' like(s)') as [file_score], fi.file_datetime, fi.file_uploader, (select em.employee_fullname from dbo.man_employee em where em.employee_id = fi.file_uploader) as [employee_fullname], (select em.employee_nickname from dbo.man_employee em where em.employee_id = fi.file_uploader) as [employee_nickname], '' as [file_view] from db_universe_erp_file.dbo.sto_file fi where (fi.file_parent = '{0}' and fi.file_filetype = 'jpg') order by fi.file_score desc, fi.file_datetime;", varContentId)
            ...
            ElseIf dbengine = LibApp.Ingrid.Global.DatabaseEngine.MYSQL Then
                varDatabaseRequestMysql(4).Query = $"select fi.file_id, fi.file_filename, fi.file_content, (convert(varchar(25),fi.file_content_size) + ' KB') as `file_content_size`, (convert(varchar(3),fi.file_score) + ' like(s)') as `file_score`, fi.file_datetime, fi.file_uploader, (select em.employee_fullname from dbo.man_employee em where em.employee_id = fi.file_uploader) as `employee_fullname`, (select em.employee_nickname from man_employee em where em.employee_id = fi.file_uploader) as `employee_nickname`, '' as `file_view` from sto_file fi where (fi.file_parent = '{varContentId}' and fi.file_filetype = 'jpg') order by fi.file_score desc, fi.file_datetime"
        ...
        Public Shared Sub DisplayFileGrid(databasename As String, dbengine As LibApp.Ingrid.Global.DatabaseEngine, contentid As String, filegrid As CMCv.UI.Control.dgn, Optional recordyear As String = "")
            ...
            If dbengine = LibApp.Ingrid.Global.DatabaseEngine.MSSQL Then
                varDatabaseRequestMssql2008(5).Query = String.Format("select fi.file_id, fi.file_filename, fi.file_tag, '' as [file_content], (convert(varchar(25),fi.file_content_size) + ' KB') as [file_content_size], (convert(varchar(3),fi.file_score) + ' like(s)') as [file_score], fi.file_datetime, fi.file_uploader, (select em.employee_fullname from dbo.man_employee em where em.employee_id = fi.file_uploader) as [employee_fullname], (select em.employee_nickname from dbo.man_employee em where em.employee_id = fi.file_uploader) as [employee_nickname], '' as [file_view] from db_universe_erp_file.dbo.sto_file fi where (fi.file_parent = '{0}' and fi.file_filetype = 'pdf') order by fi.file_datetime;", varContentId)
            ...
            ElseIf dbengine = LibApp.Ingrid.Global.DatabaseEngine.MYSQL Then
                varDatabaseRequestMysql(5).Query = $"select fi.file_id, fi.file_filename, fi.file_tag, '' as `file_content`, (convert(varchar(25),fi.file_content_size) + ' KB') as `file_content_size`, (convert(varchar(3),fi.file_score) + ' like(s)') as [file_score], fi.file_datetime, fi.file_uploader, (select em.employee_fullname from dbo.man_employee em where em.employee_id = fi.file_uploader) as [employee_fullname], (select em.employee_nickname from dbo.man_employee em where em.employee_id = fi.file_uploader) as `employee_nickname`, '' as `file_view` from db_universe_erp_file.dbo.sto_file fi where (fi.file_parent = '{varContentId}' and fi.file_filetype = 'pdf') order by fi.file_datetime;"
        ...
        Public Shared Function DeleteData(databasename As String, dbengine As LibApp.Ingrid.Global.DatabaseEngine, rowid As String) As Boolean
            ...
            Try
                ...
                ElseIf dbengine = LibApp.Ingrid.Global.DatabaseEngine.MYSQL Then
                    varDatabaseRequestMysql(1).Query = $"delete from doc_employeeactivity where employeeactivity_id = '{rowid}';delete from sto_file where file_parent = '{rowid}';"
        ...
        Public Shared Function IsLike(databasename As String, dbengine As LibApp.Ingrid.Global.DatabaseEngine, fileid As String, eid As String) As Boolean
            ...
            Try
                If dbengine = LibApp.Ingrid.Global.DatabaseEngine.MSSQL Then
                    varDatabaseRequestMssql2008(1).Query = String.Format("select count(ff.filefeedback_id) as [islike] from db_universe_erp_file.dbo.sto_filefeedback ff where ff.filefeedback_file = '{0}' and ff.filefeedback_employee = '{1}';", fileid, eid)
                    varResult = CType(varDatabaseEngineMssql2008.GetValue(databasename, varDatabaseRequestMssql2008(1).Query), Integer)
                ElseIf dbengine = LibApp.Ingrid.Global.DatabaseEngine.MYSQL Then
                    varDatabaseRequestMysql(1).Query = $"select count(ff.filefeedback_id) as `islike` from sto_filefeedback ff where ff.filefeedback_file = '{fileid}' and ff.filefeedback_employee = '{eid}';"
        ...
        Public Shared Function LikePhoto(databasename As String, dbengine As LibApp.Ingrid.Global.DatabaseEngine, fileid As String, eid As String, fileowner As String) As Boolean
            ...
            Try
                If dbengine = LibApp.Ingrid.Global.DatabaseEngine.MSSQL Then
                    varDatabaseRequestMssql2008(1).Query = String.Format("insert into db_universe_erp_file.dbo.sto_filefeedback(filefeedback_datetime, filefeedback_file, filefeedback_employee, filefeedback_type, filefeedback_value, filefeedback_text) values(GETDATE(), '{0}', '{1}', 'Like', 1, 'N/A'); insert into dbo.[[sys]]notification](notification_datetime, notification_employee, notification_message, notification_isread) values(GETDATE(), '{2}', (select em.employee_fullname from dbo.man_employee em where em.employee_id = '{1}') + ' like one of your photo.', 0);update db_universe_erp_file.dbo.sto_file set file_score = (select count(ff.filefeedback_value) from db_universe_erp_file.dbo.sto_filefeedback ff where (ff.filefeedback_file = '{0}') and (ff.filefeedback_type = 'Like')) where (file_id = '{0}');", fileid, eid, fileowner)
                    varDatabaseEngineMssql2008.PushData(databasename, varDatabaseRequestMssql2008(1).Query)
                ElseIf dbengine = LibApp.Ingrid.Global.DatabaseEngine.MYSQL Then
                    varDatabaseRequestMysql(1).Query = $"insert into db_universe_erp_file.dbo.sto_filefeedback(filefeedback_datetime, filefeedback_file, filefeedback_employee, filefeedback_type, filefeedback_value, filefeedback_text) values(GETDATE(), '{fileid}', '{eid}', 'Like', 1, 'N/A'); insert into dbo.[[sys]]notification](notification_datetime, notification_employee, notification_message, notification_isread) values(GETDATE(), '{fileowner}', (select em.employee_fullname from dbo.man_employee em where em.employee_id = '{eid}') + ' like one of your photo.', 0);update db_universe_erp_file.dbo.sto_file set file_score = (select count(ff.filefeedback_value) from db_universe_erp_file.dbo.sto_filefeedback ff where (ff.filefeedback_file = '{fileid}') and (ff.filefeedback_type = 'Like')) where (file_id = '{fileid}');"
                    varDatabaseEngineMysql.PushData(databasename, varDatabaseRequestMysql(1).Query)
                End If
                varSuccess = True
            Catch ex As Exception
        ...
        ' Editor and Reports classes unchanged (omitted for brevity in this diff)
        Public Class Editor
